...
public class ObstacleSprite2 implements GameObject
{
    ...
    public ObstacleSprite2(Bitmap bmp,int xStart)
    {
        image = bmp;
        x = xStart;
        y = 0;
    ...
    public boolean bulletCollide(BulletManager manager, ArrayList<Rect> bulletDecision, Rect enemy)
    {
        ...
        if(manager.getBullets().size() == 0){
            ...
            for ( i = 0; i < manager.getBullets().size(); i++) {
                ...
                Rect detect1 = new Rect();
                //Rect detect2 = new Rect();

                detect1.set(manager.getBullets().get(i).getLeft(), manager.getBullets().get(i).getTop(),
...
//                        manager.getBulletis().get(i).getRight(), manager.getBulletis().get(i).getBottom());
            ...
            //i = 0;
            for ( i = 0; i < manager.getBulletiis().size(); i++) {

                Rect detect = new Rect();
                //Rect detect2 = new Rect();

                detect.set(manager.getBulletiis().get(i).getLeft(), manager.getBulletiis().get(i).getTop(),
                        manager.getBulletiis().get(i).getRight(), manager.getBulletiis().get(i).getBottom());
